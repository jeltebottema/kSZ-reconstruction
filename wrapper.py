...
from py21cmfast._utils import OutputStruct, _check_compatible_inputs, _process_exitcode
from py21cmfast.c_21cmfast import ffi, lib
from py21cmfast.cache_tools import get_boxes_at_redshift
...
from py21cmfast.lightcones import Lightconer, RectilinearLightconer
from py21cmfast.outputs import (
    AngularLightcone,
    BrightnessTemp,
    Coeval,
    HaloField,
...
#            "velocity_x",
    ...
    # If l_s is just a scalar, and you need to assign a unit (e.g., Mpc):
    # l_s = l_s * un.Mpc  # Assigning the unit to l_s
...
#            "velocity_x",
    ...
    P_k = P_k/ (2 * np.pi) * l_s** 2* Planck18.Tcmb0.value**4
    err = np.sqrt(err) * l_s ** 2    

    return KSZOutput(
        (Tcmb * kSZ_consts.CMperMPC / constants.c.cgs.value * Planck18.Tcmb0.value)**2,
        mean_taue_fin,
        l_s=l_s[np.logical_not(np.isnan(l_s))],
        kSZ_power=P_k[np.logical_not(np.isnan(l_s))],
        err=err[np.logical_not(np.isnan(l_s))],
...
def run_kSZ_tryout(
    ...
    """
    Run kSZ and 21cm projection using 21cmFAST lightcone.
    Adds:
    - Power spectrum of 21cm brightness temperature map
    - Cross-power spectrum between kSZ and 21cm fields
    - Option to square kSZ and/or Tb maps for cross-correlation
    """
    if lc:
        user_params = lc.user_params
        cosmo_params = lc.cosmo_params
        ...
        logger.warning("run_kSZ requires a lightcone object, which is not given. Running with default parameters.")

    random.seed(random_seed)

    # === Optional redshift cutoff ===
    redshifts = np.array(lc.lightcone_redshifts)
    density = lc.density
    velocity = lc.velocity
    xH_box = lc.xH_box
    Tb_box = lc.brightness_temp

    n_slices = density.shape[2]
    redshifts = redshifts[:n_slices]  # Now redshifts and cubes are aligned

    if z_cut is not None:
        if isinstance(z_cut, tuple):
            mask = (redshifts >= z_cut[0]) & (redshifts <= z_cut[1])
        else:
            mask = redshifts <= z_cut
    ...
    Tcmb, mean_taue_fin = _Proj_array(
        ...
        velocity=velocity,
        xH=xH_box,
        kSZ_consts=kSZ_consts,
        PARALLEL_APPROX=PARALLEL_APPROX,
        rotation=rotation,
    )

    # === 21cm projection ===
    Tb_proj_map, _ = project_21cm_like_kSZ(
        redshifts=redshifts,
        Tb_21cm=Tb_box,
        density=density,
        xH=xH_box,
    ...
    print(Tb_proj_map.shape)
    # === Power spectrum of kSZ ===
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        P_k_kSZ, l_s_kSZ, err_kSZ = get_power(
            deltax=Tcmb * kSZ_consts.CMperMPC / constants.c.cgs.value,
            boxlength=lc.user_params.BOX_LEN,
            bins=bins,
            log_bins=True,
            get_variance=True,
        )

    l_s_kSZ *= lc.lightcone_distances[0].value
    mask_kSZ = np.logical_not(np.isnan(l_s_kSZ))
    P_k_kSZ = P_k_kSZ[mask_kSZ] / (2 * np.pi) * l_s_kSZ[mask_kSZ]**2 * Planck18.Tcmb0.value**2
    err_kSZ = np.sqrt(err_kSZ[mask_kSZ]) * l_s_kSZ[mask_kSZ]**2

    # === Power spectrum of 21cm brightness temperature ===
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        P_k_21cm, l_s_Tb, err_21cm = get_power(
            deltax=Tb_proj_map,
            boxlength=lc.user_params.BOX_LEN,
            bins=bins,
            log_bins=True,
    ...
    l_s_Tb *= lc.lightcone_distances[0].value
    mask_Tb = np.logical_not(np.isnan(l_s_Tb))
    P_k_21cm = P_k_21cm[mask_Tb] / (2 * np.pi) * l_s_Tb[mask_Tb]**2
    err_21cm = np.sqrt(err_21cm[mask_Tb]) * l_s_Tb[mask_Tb]**2

    # === Prepare maps for cross-power ===
    # === Prepare maps for cross-power ===
    # === Prepare maps for cross-power ===
    kSZ_map_for_cross = Tcmb
    Tb_map_for_cross = Tb_proj_map

    if square_kSZ:
        kSZ_map_for_cross = kSZ_map_for_cross**2
    if square_Tb:
        Tb_map_for_cross = Tb_map_for_cross**2


    # === Cross-power spectrum between kSZ and 21cm ===
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        P_k_cross, l_s_cross, err_cross = get_power(
            deltax=kSZ_map_for_cross * kSZ_consts.CMperMPC / constants.c.cgs.value,
            deltax2=Tb_map_for_cross,
            boxlength=lc.user_params.BOX_LEN,
            bins=bins,
            log_bins=True,
            get_variance=True,
    ...
    l_s_cross *= lc.lightcone_distances[0].value
    mask_cross = np.logical_not(np.isnan(l_s_cross))
    P_k_cross = P_k_cross[mask_cross] / (2 * np.pi) * l_s_cross[mask_cross]**2 * Planck18.Tcmb0.value
    err_cross = np.sqrt(err_cross[mask_cross]) * l_s_cross[mask_cross]**2


    return KSZ_21cmOutput(
        kSZ_map=Tcmb * kSZ_consts.CMperMPC / constants.c.cgs.value * Planck18.Tcmb0.value,
        mean_tau=mean_taue_fin,
        l_s_kSZ=l_s_kSZ[mask_kSZ],
        kSZ_power=P_k_kSZ,
        kSZ_err=err_kSZ,
        l_s_Tb=l_s_Tb[mask_Tb],
        Tb_power=P_k_21cm,
        Tb_err=err_21cm,
        l_s_cross=l_s_cross[mask_cross],
        cross_power=P_k_cross,
        cross_err=err_cross,
        brightness_temp_box=Tb_proj_map
    )
...
def _Proj_array(
    ...
    Tcmb = Tcmb - np.mean(Tcmb) #why not divide by Tcmb as well
    return Tcmb, mean_taue_fin

def project_21cm_like_kSZ(
    redshifts,
    Tb_21cm,
    density,
    xH,
    ...
    """
    Project 21cm brightness temperature field using same ray tracing and projection method as kSZ (_Proj_array).

    Parameters:
        redshifts : array of redshifts per slice (Nz,)
        Tb_21cm   : 3D brightness temperature cube (Nx, Ny, Nz) in mK
        density   : 3D overdensity field (Nx, Ny, Nz), required for dtau computation
        xH        : 3D neutral fraction field (Nx, Ny, Nz)
        kSZ_consts: kSZ constants object for geometry and tau prefactors
        PARALLEL_APPROX : bool, optional
            Same logic as in _Proj_array
        rotation : bool, optional
            Same logic as in _Proj_array

    Returns:
        Tb_proj: 2D projected 21cm map (same shape as Tcmb output in _Proj_array)
        dummy_tau: set to 0.0, for compatibility
    """
    import random

    dtau_3d = kSZ_consts.A * (1.0 + density) * (1.0 + kSZ_consts.Y_He / 4 - xH)
    signal_3d = Tb_21cm * (1 + redshifts)  # Optional redshift weighting

    if not (PARALLEL_APPROX or rotation):
        taue_arry = np.cumsum(dtau_3d * (1 + redshifts) ** 2, axis=2) + kSZ_consts.mean_taue_curr_z
        Tb_proj = signal_3d * np.exp(-taue_arry)
        Tb_proj = np.sum(Tb_proj, axis=2)
        taue_arry = taue_arry[:, :, -1]
    else:
        inc = 1
        inc_displacement = kSZ_consts.dR / kSZ_consts.DA_zstart
        Tb_proj = np.zeros((kSZ_consts.HII_DIM, kSZ_consts.HII_DIM))
        ...
        for k in range(kSZ_consts.red_dist):
            ...
            if not PARALLEL_APPROX:
                ...
                inc += inc_displacement.value
                dtau_new = np.take(dtau_new, a, axis=0, mode="wrap")
                dtau_new = np.take(dtau_new, a, axis=1, mode="wrap")
                Tb_new = np.take(Tb_new, a, axis=0, mode="wrap")
                Tb_new = np.take(Tb_new, a, axis=1, mode="wrap")

            if rotation:
                if k % kSZ_consts.HII_DIM == 0:
                    tx = int(kSZ_consts.HII_DIM * random.random())
                    ty = int(kSZ_consts.HII_DIM * random.random())
                dtau_new = np.roll(dtau_new, -tx, axis=0)
                dtau_new = np.roll(dtau_new, -ty, axis=1)
                Tb_new = np.roll(Tb_new, -tx, axis=0)
                Tb_new = np.roll(Tb_new, -ty, axis=1)

            taue_arry += dtau_new
            Tb_proj += Tb_new * np.exp(-taue_arry)

    Tb_proj -= np.mean(Tb_proj)
...
class KSZ_21cmOutput:
    """Output class for kSZ and 21cm results with separate multipole arrays."""
    ...
    def __init__(
        ...
        cross_power=None,
        cross_err=None,
        brightness_temp_box=None,
        cosmo_params=None
    ):
        self.kSZ_map = kSZ_map
        self.mean_tau = mean_tau
        self.l_s_kSZ = l_s_kSZ
        self.kSZ_power = kSZ_power
        self.kSZ_err = kSZ_err
        self.l_s_Tb = l_s_Tb
        self.Tb_power = Tb_power
        self.Tb_err = Tb_err
        self.l_s_cross = l_s_cross
        self.cross_power = cross_power
        self.cross_err = cross_err
        self.brightness_temp_box = brightness_temp_box
        self.cosmo_params = cosmo_params
